...
export default function PasswordForm({ visible, onClose, onSubmit }: PasswordFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 animate-fadeIn">
      <div className="relative w-[450px] rounded-lg bg-white/20 backdrop-blur shadow-2xl p-8 flex flex-col gap-5 items-center animate-fadeIn">
        ...
        <input type="password" value={inputPw} onChange={e => setInputPw(e.target.value)}
          className="w-full px-4 py-3 rounded-md border-none outline-none bg-white/30 focus:bg-white/60 backdrop-blur text-base font-medium placeholder:text-gray-300 transition"
